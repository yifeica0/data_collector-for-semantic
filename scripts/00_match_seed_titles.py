...
def main() -> None:
    ...
    for r in seeds.itertuples(index=False):
        ...
        paper_obj = candidates[0] if isinstance(candidates, list) and candidates else (data if isinstance(data, dict) and data.get("paperId") else None)
        if paper_obj and paper_obj.get("paperId"):
            paper = dict(paper_obj)
            paper_rows.append(paper)
            rows.append({
                "seed_id": getattr(r, "seed_id", ""),
                "input_title": title,
                "topic": getattr(r, "topic", ""),
                "paperId": paper_obj.get("paperId"),
